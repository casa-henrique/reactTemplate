    ...
    return <Container>
        ...
        <div className="trialItemsWrapper">
            {activitiesSeasonOne?.map((item:any, index:any) => {
              return <TrialItem key={index} id={item.id} title={item.main ? 'Atividade principal' : item.name} access={item.name} type="activity"/>
            })}
        </div>
        ...
        <div className="trialItemsWrapper">
         ...
         activitiesSeasonTwo.length > 0 ? activitiesSeasonTwo.map((item:any, index:any) => {
          return <TrialItem key={index} id={item.id} title={item.main ? 'Atividade principal' : item.name.replaceAll("Atividade secundaria Apresentacao", "Atividade secundária: Apresentação")} access={item.name} type="activity"/>
      }) : <TrialItem title='Em breve' access="" type="soon"/>
        }
        <TrialItem title='Atividade secundária: Mundo e cultura digital' access="" type="soon"/>
        </div>
    </Container>
}